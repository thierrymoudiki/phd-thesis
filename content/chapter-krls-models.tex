...
The following figures, \ref{oos_rmse_krls} and \ref{oos_rmse_dns_krls},  present the out-of-sample RMSE obtained over time by all the KRLS models, for horizons equal to $12$ and $36$. On figure \ref{oos_rmse_krls} (on the right), we observe that despite being the best model on average, the Mat\'ern 3/2 is not necessarily the best during the liquidity/credit crunch (especially in 2008) for an horizon of $36$ months. On figure \ref{oos_rmse_dns_krls}, we observe that all the kernels lead to very similar results, with only slight differences. During the crisis, and for an horizon of $36$ months, despite displaying more volatile and less accurate results elsewhere, the DNS-ARIMA is performing better than the other models.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=13cm]{gfx/chapter-krls-models/krls_oos_rmse_2}
...
\end{verbatim}
=======
We use the same grids for all the models, in order to ease the comparability, and to avoid too much manual tuning for the optimal set of parameters. For both types of models, we consider $5$ values of $\sigma$, $l$ and $\lambda$ (kernels' parameters and regularization parameters) comprised between and for the DNS-KRLS model, we use 5 values $\eta$ comprised between the minimum observed time to maturity and the maximum observed time to maturity.
