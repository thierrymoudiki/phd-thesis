...
The elements $W^{(j, l)}$ of the hidden layer are taken from a quasi-random (deterministic) \textit{sobol} sequence on $[0, 1]$, which is shifted in such a way that they belong to $[-1, 1]$. In the case of bagging and stacking, solving for the optimal $\beta_j$'s and $\gamma_l$'s is done by searching these parameters,  in restricted regions where we have:
$$
...
\sum_{l=1}^L
\gamma_l^2 \leq v
$$ for $u, v \in \RR^*$. That is, by applying a regularization to these unknown parameters. In the case of boosting, we do not restrict the regression parameters, but use the boosting algorithm described in (cite Hothorn et al. 2010). 


\medskip

If we consider $p \in \mathbb{N}^*$ time series $(X_t^{(j)})_{t \geq 0}, j = 1, \ldots, p$,
...
where $\lambda_1$ and $\lambda_2$ are Lagrange multipliers. Otherwise, without the constraints, the problem to be solved is simply a least squares regression problem on the augmented dataset. By denoting:
...
As mentioned in the introduction, we will use bagging, boosting, and stacking to construct ensembles of RVFL from the previous section \ref{sec:basemodeldesc}. These techniques will be described in details in the next sections ~\ref{sec:rvflbagging}, ~\ref{sec:rvflboosting}  and \ref{sec:rvflstacking}.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=11.5cm]{gfx/chapter-rvfl-ensembles/rvfl_stacking.png}
