...
%
\chapter{A Bayesian Quasi-Randomized neural networks, and its application to the optimization of black box functions}
...
The RVFL networks have been successfully applied to solving different types of classification and regression problems; see for example \cite{dehuri2010comprehensive}. Here, we will use RVFL networks with only one hidden layer. And instead of relying on fully randomized nodes, we will use sequences of deterministic quasi-random numbers. 
...
$$
where $g$ is an activation function, as in of neural networks models, and $W \in \RR^{p \times L}$ are obtained from a deterministic sobol sequence of quasi-random numbers (cite Niderreiter (1992) and Boyle et Tan (1997)). $L$ is the number of nodes in the hidden layer. 
...
$$
It is possible to obtain a sensitivity (first order) of the response to a change in the covariates. Indeed, in this case, we have for $i \in \left \lbrace 1, \ldots, n \right \rbrace $: 

$$
y_i = \sum_{j = 1}^p \alpha_j X_i^{(j)} + \sum_{k = 1}^L \gamma_k g(X_i^T W^{(k)})
$$

and for a fixed integer $j_0 \in \left \lbrace 1, \ldots, p \right \rbrace$, and $u := X_i^T W^{(k)}$, we have: 
...
\begin{figure}[!htb]
    ...
    \begin{minipage}{.5\textwidth}
        ...
        \caption{Out-of-sample predictions and $90$\%, $95$\% confidence intervals for SLC14.1 (with \code{seed = 123})}
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        \caption{Out-of-sample predictions and $90$\%, $95$\% confidence intervals for SLC14.2 (with \code{seed = 123})}
    ...
    \begin{minipage}{.5\textwidth}
        ...
        \caption{Out-of-sample predictions and $90$\%, $95$\% confidence intervals for SLC14.1 (with \code{seed = 456})}
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        \caption{Out-of-sample predictions and $90$\%, $95$\% confidence intervals for SLC14.2 (with \code{seed = 456})}
...
\subsection{Example of DNS-KRLS vs DNS-RVFL}
...
\begin{center}
% table caption is above the table
\caption{out-of-sample RMSE for data-driven models}
